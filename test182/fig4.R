...
file <- paste('~/fig4','_',dir,'.pdf',sep="")
  pdf(file,width=14,height=6)
###
par(mar=c(3,4,2,4),mfrow=c(2,4),las=1,xaxs='i',yaxs='i')
  require('pomp.orig')
  require('pracma')
...
points(times,5+3*(0.99-SH/y$bpop[1]),type='l',lwd=2,col='brown')
points(times,5+3*(BC/(y$bpop[1])),type='l',lwd=1,col='darkgreen')
points(times[seq(1,89,by=5)],5+3*(BC[seq(1,89,by=5)]/(y$bpop[1])),lty=1,pch=2,col='darkgreen')
...
print(y[1,])
if(p %in% c(1,4)) mtext(side=3,line=-1,adj=0.05,ifelse(p<2,'Manaus','Iquitos'))
}
if(TRUE){
for(p in c(3:4)){
...
plot(x$psi,x$loglik-max(x$loglik),type='b',pch=1,log='',ylab='log likelihood',xlab='',ylim=c(ifelse(p==3,-8,-7),0),lwd=2)
mtext(side=1,line=2.5,expression(psi))
mtext(side=2,line=3,adj=0.3,expression(Delta),las=0)
abline(h=max(x$loglik)-0.5*qchisq(0.95,1),lty=2)
mtext(side=1,line=3,adj=0.7,'')
par(new=T)
plot(x$psi,x$theta*x$theta*100,type='b',col='red',log='',pch=2,ann=F,axe=F,frame=T,ylim=c(0.25, 1.2))
abline(h=0.5,col='lightgrey',lty=2)
points(x$psi,x$alpha*x$theta*x$theta*100,type='b',col='blue',pch=3)
axis(4)
mtext(side=4,line=3,las=0,'IFR (%)')
  mtext(side=3,line=0.5,adj=-0.15,text=letters[ifelse(p==3,4,8)],font=2,cex=1)
if(p==3)
{
legend("bottomleft",bty='n',pch=1:3,col=c('black','red','blue'),legend=c('  log likelihood','IFR1','IFR2'),lty=1,lwd=c(1,1,1))
mtext(side=1,line=-3.5,adj=0.11,expression(Delta))
}
}
}
